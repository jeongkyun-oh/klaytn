...
var (
	...
	//snapshotBloomIndexTimer = metrics.NewRegisteredResettingTimer("state/snapshot/bloom/index", nil) TODO-Klaytn-Snapshot check metrics
...
type Tree struct {
	diskdb database.Database        // Persistent database to store the snapshot
...
func New(diskdb database.Database, triedb *statedb.Database, cache int, root common.Hash, async bool, rebuild bool, recovery bool) (*Tree, error) {
...
func (t *Tree) Disable() {
	...
	database.WriteSnapshotDisabled(batch)
	database.DeleteSnapshotRoot(batch)
	database.DeleteSnapshotJournal(batch)
	database.DeleteSnapshotGenerator(batch)
	database.DeleteSnapshotRecoveryNumber(batch)
...
func diffToDisk(bottom *diffLayer) *diskLayer {
	...
	for hash := range bottom.destructSet {
		...
		database.DeleteAccountSnapshot(batch, hash)
		base.cache.Set(hash[:], nil)

		it := database.IterateStorageSnapshots(base.diskdb, hash)
		for it.Next() {
	...
	for hash, data := range bottom.accountData {
		...
		database.WriteAccountSnapshot(batch, hash, data)
	...
	for accountHash, storage := range bottom.storageData {
		...
		for storageHash, data := range storage {
			...
			if len(data) > 0 {
				database.WriteStorageSnapshot(batch, accountHash, storageHash, data)
				base.cache.Set(append(accountHash[:], storageHash[:]...), data)
				snapshotCleanStorageWriteMeter.Mark(int64(len(data)))
			} else {
				database.DeleteStorageSnapshot(batch, accountHash, storageHash)
	...
	database.WriteSnapshotRoot(batch, bottom.root)
...
func (t *Tree) Journal(root common.Hash) (common.Hash, error) {
	...
	database.WriteSnapshotJournal(t.diskdb, journal.Bytes())
	return base, nil
}
...
func (t *Tree) Rebuild(root common.Hash) {
	...
	database.DeleteSnapshotRecoveryNumber(t.diskdb)
	database.DeleteSnapshotDisabled(t.diskdb)
...
func (t *Tree) Verify(root common.Hash) error {
	...
	got, err := generateTrieRoot(nil, acctIt, common.Hash{}, stackTrieGenerate, func(db database.KeyValueWriter, accountHash, codeHash common.Hash, stat *generateStats) (common.Hash, error) {
