...
func (gs *generatorStats) Log(msg string, root common.Hash, marker []byte) {
	...
	//log.Info(msg, ctx...) TODO-Klaytn-Snapshot add logger for snapshot package
}

// generateSnapshot regenerates a brand new snapshot based on an existing state
// database and head block asynchronously. The snapshot is returned immediately
// and generation is continued in the background until done.
func generateSnapshot(diskdb database.Database, triedb *statedb.Database, cache int, root common.Hash) *diskLayer {
	...
	database.WriteSnapshotRoot(batch, root)
...
func journalProgress(db database.KeyValueWriter, marker []byte, stats *generatorStats) {
	...
	database.WriteSnapshotGenerator(db, blob)
}
...
func (dl *diskLayer) proveRange(stats *generatorStats, root common.Hash, prefix []byte, kind string, origin []byte, max int, valueConvertFn func([]byte) ([]byte, error)) (*proofResult, error) {
	...
	if origin == nil && !diskMore {
		stackTr := statedb.NewStackTrie(nil)
...
func (dl *diskLayer) generateRange(root common.Hash, prefix []byte, kind string, origin []byte, max int, stats *generatorStats, onState onStateCallback, valueConvertFn func([]byte) ([]byte, error)) (bool, []byte, error) {
	...
	//var snapNodeCache database.Database
	//if len(result.keys) > 0 {
	...
	//	snapTrieDb.Commit(root, false, -1)
	//}
	tr := result.tr
	if tr == nil {
		tr, err = statedb.NewTrie(root, dl.triedb)
...
func (dl *diskLayer) generate(stats *generatorStats) {
	...
	onAccount := func(key []byte, val []byte, write bool, delete bool) error {
		...
		if delete {
			database.DeleteAccountSnapshot(batch, accountHash)
		...
		if accMarker == nil || !bytes.Equal(accountHash[:], accMarker) {
			...
			if !write {
				...
				dataLen = len(data)
				database.WriteAccountSnapshot(batch, accountHash, data)
		...
		if acc.Root == emptyRoot {
			...
			onStorage := func(key []byte, val []byte, write bool, delete bool) error {
				...
				if delete {
					database.DeleteStorageSnapshot(batch, accountHash, common.BytesToHash(key))
					snapWipedStorageMeter.Mark(1)
					return nil
				}
				if write {
					database.WriteStorageSnapshot(batch, accountHash, common.BytesToHash(key), val)
