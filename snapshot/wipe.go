...
func wipeSnapshot(db database.Database, full bool) chan struct{} {
	// Wipe the snapshot root marker synchronously
	if full {
		database.DeleteSnapshotRoot(db)
	}
...
func wipeContent(db database.Database) error {
	//if err := wipeKeyRange(db, "accounts", database.SnapshotAccountPrefix, nil, nil, len(database.SnapshotAccountPrefix)+common.HashLength, snapWipedAccountMeter, true); err != nil {
	//	return err
	//}
	//if err := wipeKeyRange(db, "storage", database.SnapshotStoragePrefix, nil, nil, len(database.SnapshotStoragePrefix)+2*common.HashLength, snapWipedStorageMeter, true); err != nil {
	//	return err
	//}
	...
	//end[len(end)-1]++
	//
	//if err := db.Compact(database.SnapshotAccountPrefix, end); err != nil {
	//	return err
	//}
	...
	//end[len(end)-1]++
	//
	//if err := db.Compact(database.SnapshotStoragePrefix, end); err != nil {
	//	return err
	//}
...
func wipeKeyRange(db database.Database, kind string, prefix []byte, origin []byte, limit []byte, keylen int, meter metrics.Meter, report bool) error {
