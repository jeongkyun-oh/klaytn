...
func loadAndParseJournal(db database.Database, base *diskLayer) (snapshot, journalGenerator, error) {
	// Retrieve the disk layer generator. It must exist, no matter the
	// snapshot is fully generated or not. Otherwise the entire disk
	// layer is invalid.
	generatorBlob := database.ReadSnapshotGenerator(db)
	...
	journal := database.ReadSnapshotJournal(db)
	if len(journal) == 0 {
...
func loadSnapshot(diskdb database.Database, triedb *statedb.Database, cache int, root common.Hash, recovery bool) (snapshot, bool, error) {
	// If snapshotting is disabled (initial sync in progress), don't do anything,
	// wait for the chain to permit us to do something meaningful
	if database.ReadSnapshotDisabled(diskdb) {
		return nil, true, nil
	}
	// Retrieve the block number and hash of the snapshot, failing if no snapshot
	// is present in the database (or crashed mid-update).
	baseRoot := database.ReadSnapshotRoot(diskdb)
