...
func makeKafkaConfig(ctx *cli.Context) *kafka.KafkaConfig {
	...
	if ctx.GlobalIsSet(utils.ChainDataFetcherKafkaBrokersFlag.Name) {
		kafkaConfig.BrokerList = ctx.GlobalStringSlice(utils.ChainDataFetcherKafkaBrokersFlag.Name)
	} else {
		logger.Crit("The kafka brokers must be set")
	}
	kafkaConfig.GroupID = ctx.GlobalString(utils.ChainDataFetcherKafkaConsumerGroupIdFlag.Name)
	...
	kafkaConfig.TopicPrefix = ctx.GlobalString(utils.ChainDataFetcherKafkaTopicPrefixFlag.Name)
	return kafkaConfig
}
