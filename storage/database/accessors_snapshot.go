...
func ReadSnapshotDisabled(db KeyValueReader) bool {
...
func ReadSnapshotRoot(db KeyValueReader) common.Hash {
...
func ReadAccountSnapshot(db KeyValueReader, hash common.Hash) []byte {
...
func ReadStorageSnapshot(db KeyValueReader, accountHash, storageHash common.Hash) []byte {
...
func IterateStorageSnapshots(db Iteratee, accountHash common.Hash) Iterator {
	return db.NewIterator(storageSnapshotsKey(accountHash), nil)
}

// ReadSnapshotJournal retrieves the serialized in-memory diff layers saved at
// the last shutdown. The blob is expected to be max a few 10s of megabytes.
func ReadSnapshotJournal(db KeyValueReader) []byte {
...
// the last shutdown.
func ReadSnapshotGenerator(db KeyValueReader) []byte {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db KeyValueReader) *uint64 {
...
func ReadSnapshotSyncStatus(db KeyValueReader) []byte {
	data, _ := db.Get(snapshotSyncStatusKey)
	return data
}

// WriteSnapshotSyncStatus stores the serialized sync status to save at shutdown.
func WriteSnapshotSyncStatus(db KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSnapshotSyncStatus(db KeyValueWriter) {
