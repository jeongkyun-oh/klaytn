...
func (valSet *weightedCouncil) Refresh(hash common.Hash, blockNum uint64, config *params.ChainConfig, governingNode common.Address) error {
	valSet.validatorMu.Lock()
	defer valSet.validatorMu.Unlock()
	...
	if chainRules.IsIstanbul {
		...
		weightedValidators, stakingAmounts, demotedValidators, _ = filterPoorValidators(weightedValidators, stakingAmounts, governingNode)
