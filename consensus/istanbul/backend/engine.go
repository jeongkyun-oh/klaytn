...
func (sb *backend) snapshot(chain consensus.ChainReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
	...
	if sb.governance.ProposerPolicy() == uint64(istanbul.WeightedRandom) {
		...
		if pHeader != nil {
			govMode := governance.GovernanceModeMap[sb.governance.GovernanceMode()]
			govNode := common.Address{}
			if govMode == params.GovernanceMode_Single {
				govNode = sb.governance.GoverningNode()
			}

			if err := snap.ValSet.Refresh(pHeader.Hash(), pHeader.Number.Uint64(), sb.chain.Config(), govNode); err != nil {
