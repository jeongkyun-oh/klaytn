...
func (sb *backend) initSnapshot(chain consensus.ChainReader) (*Snapshot, error) {
	...
	snap := newSnapshot(sb.governance, 0, genesis.Hash(), validator.NewValidatorSet(istanbulExtra.Validators, istanbulExtra.Validators, istanbul.ProposerPolicy(proposerPolicy), committeeSize, chain), chain.Config())
